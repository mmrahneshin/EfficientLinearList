...
using namespace std;
typedef PureParitySeeking23RedBlackBinaryTree<string, InternalPureParitySeeking23RedBlackBinaryTreeNode<string>> StringBinaryTree;

string inOrder[] = {"g", "d", "h", "l", "a", "e", "i", "k", "c"};
string preOrder[] = {"l", "d", "g", "h", "e", "a", "k", "i", "c"};
string postOrder[] = {"g", "h", "d", "a", "i", "c", "k", "e", "l"};
string deleted[] = {"d", "l", "i", "k", "c"};
...
bool deleteCheck(StringBinaryTree *bt)
{
    ...
    bt->deleteLeftChild(btnk);
    bt->draw(cout);
    cout << endl;
    cout << bt->getColor(btni) << endl;
    if (btni.getParent() != btnk || bt->getColor(btni) != 1)
    {
        result = false;
        return result;
    }
    ...
    cout << bt->getColor(btng) << endl;
    cout << bt->getColor(btnk) << endl;
    if (bt->getColor(btng) != 0 || bt->getColor(btnk) != 0)
    {
        result = false;
        return result;
    }
